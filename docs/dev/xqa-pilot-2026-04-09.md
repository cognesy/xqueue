...
`xqueue` now has a minimal but credible `xqa` rollout:

- `xqa doctor` for shared quality-workflow readiness
- `xqa profile run default` for the shared deterministic verification lane
- `xqa profile run style` for Ruff-only checks
- `xqa profile run architecture` for the Semgrep-backed architecture audit
- `xqa progress` for before/current/remaining-work visibility
