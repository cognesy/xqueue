...
from xqueue.adapters.sqlite.models import JobModel
from xqueue.jobs.models import JobPruneResult, JobPruneSummary, JobState
...
class JobPruningService:
    ...
    def prune(
        ...
        *,
        ...
        if dry_run:
            dry_run_log_paths = self._collect_log_paths(models) if prune_logs else []
            return JobPruneResult(
                ...
                deleted_log_count=len(dry_run_log_paths),
                deleted_log_paths=dry_run_log_paths,
    ...
    def _collect_log_paths(self, models: Sequence[JobModel]) -> list[str]:
        return [
            path
            for model in models
