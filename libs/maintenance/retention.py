"""Explicit retention cleanup adapter."""
...
from xqueue.adapters.sqlite.models import AttemptModel, EventModel, JobModel
from xqueue.jobs.models import JobState
from xqueue.maintenance.models import RetentionCleanupResult
...
class RetentionCleanupService:
    ...
    def cleanup(
        ...
        *,
        ...
        if prune_attempts and attempt_ids:
            deleted_log_paths.extend(self._collect_attempt_log_paths(list(attempts_by_id.values())))
    ...
    def _collect_attempt_log_paths(self, attempts: Sequence[AttemptModel]) -> list[str]:
        return [path for attempt in attempts for path in [attempt.stdout_path, attempt.stderr_path] if path is not None]
