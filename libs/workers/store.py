"""SQLite worker registry and atomic claim adapter."""
...
from sqlalchemy import CursorResult, Result, insert, or_, select, update
from sqlalchemy.orm import Session
from xqueue.adapters.sqlite.models import JobModel, WorkerModel
...
from xqueue.workers.models import RegisterWorkerInput, WorkerState, WorkerView


def _rowcount(result: Result[Any]) -> int:
    """A DML statement always yields a cursor result; rowcount lives there."""
...
class WorkerService:
    ...
    def list_workers(self, session: Session) -> list[WorkerView]:
        models = (
            session.execute(select(WorkerModel).order_by(WorkerModel.started_at.asc(), WorkerModel.id.asc()))
            .scalars()
            .all()
        )
        return [self.to_worker_view(model) for model in models]

    def running_worker_commands(self, session: Session) -> dict[str, str]:
        rows = session.execute(
            select(JobModel.worker_id, JobModel.command)
            .where(JobModel.state == JobState.RUNNING.value)
            .where(JobModel.worker_id.is_not(None))
        ).all()
        return {row.worker_id: row.command for row in rows}
