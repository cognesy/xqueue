"""Command contracts for AXI-oriented CLI output."""
...
def contract_from_model(
    ...
    *,
    ...
    if missing_default_fields:
        raise ValueError(
            f"contract {name!r}: default_fields {sorted(missing_default_fields)} "
            f"not present in {model_class.__name__}"
        )
    ...
    if missing_row_defaults:
        raise ValueError(
            f"contract {name!r}: default_row_fields {sorted(missing_row_defaults)} "
            f"not present in row model fields"
        )

    missing_item_defaults = set(default_item_fields) - set(item_fields)
    if missing_item_defaults:
        raise ValueError(
            f"contract {name!r}: default_item_fields {sorted(missing_item_defaults)} "
            f"not present in item model fields"
        )
...
def validate_requested_fields(contract: CommandContract, requested: tuple[str, ...]) -> tuple[str, ...]:
    ...
    if invalid:
        raise ValueError(
            f"unknown fields for {contract.name}: {', '.join(invalid)}; "
