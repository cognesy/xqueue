"""Queue inspection and control services."""
...
class QueueService:
    ...
    def list_queues(self, session: Session) -> list[QueueView]:
        ...
        return [
            self._to_queue_view(row_map.get(name), name=name)
            for name in names
        ]
        ...
        counts = {
            ...
            for row in session.execute(
                select(
                    ...
                    func.sum(case((JobModel.state == JobState.RETRY_SCHEDULED.value, 1), else_=0)).label("retry_scheduled_jobs"),
    ...
    def purge_jobs(self, session: Session, *, queue: str) -> PurgeJobsResult:
        ...
        job_ids = list(
            session.execute(
                select(JobModel.id)
                .where(JobModel.queue == queue)
                .where(JobModel.state.in_(purgeable_states))
            ).scalars().all()
        )
        if not job_ids:
