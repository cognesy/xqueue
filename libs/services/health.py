"""Operational health and diagnostic services."""
    ...
    def collect_health(
        ...
        *,
        ...
        if session is not None and database_check.status is not HealthStatus.ERROR:
            paused_queues = [item.name for item in self._queue_service.list_queues(session) if item.state.value == "paused"]
    ...
    def _list_stale_workers(
        ...
        *,
        ...
        models = session.execute(
            select(WorkerModel)
            .where(WorkerModel.state.in_([WorkerState.ACTIVE.value, WorkerState.PAUSED.value, WorkerState.DRAINING.value]))
