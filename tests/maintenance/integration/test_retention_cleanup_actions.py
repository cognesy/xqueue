...
from xqueue.maintenance.actions import CleanupRetentionAction
...
def test_cleanup_retention_action_prunes_only_old_terminal_log_artifacts(tmp_path: Path) -> None:
    ...
    assert result.deleted_attempt_count == 0
    assert result.deleted_event_count == 0
    assert result.deleted_log_count == 2
    assert sorted(Path(path).name for path in result.deleted_log_paths) == ["old.stderr.log", "old.stdout.log"]
