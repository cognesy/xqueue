...
def test_recover_stale_leases_requeues_job_and_records_event(tmp_path: Path) -> None:
    ...
    with SessionManager(session_factory).transaction() as session:
        session.add(WorkerModel(id="worker-1", state="active", queues=["alpha"], heartbeat_at=now - timedelta(minutes=2), started_at=now))
        session.add(
            JobModel(
                id="job-stale",
                queue="alpha",
    ...
    assert result.item.items[0].job_id == "job-stale"
    assert result.item.items[0].new_state == JobState.RETRY_SCHEDULED
    assert result.item.items[0].available_at == now + timedelta(seconds=15)
...
def test_recover_stale_leases_marks_job_failed_when_attempts_are_exhausted(tmp_path: Path) -> None:
    ...
    assert result.item.recovered_count == 1
    assert result.item.items[0].new_state == JobState.FAILED
    assert result.item.items[0].available_at is None
...
def test_health_and_doctor_surface_paused_queues_stale_leases_and_stale_workers(tmp_path: Path) -> None:
    ...
    with SessionManager(session_factory).transaction() as session:
        ...
        session.add(WorkerModel(id="worker-stale", state="active", queues=["alpha"], heartbeat_at=now - timedelta(minutes=5), started_at=now - timedelta(minutes=10)))
        session.add(
            JobModel(
                id="job-stale-health",
                queue="alpha",
    ...
    assert health_result.item.status.value == "warn"
    assert health_result.item.database_status.value == "ok"
    assert health_result.item.paused_queues == ["alpha"]
    assert [item.job_id for item in health_result.item.stale_leases] == ["job-stale-health"]
    assert [item.id for item in health_result.item.stale_workers] == ["worker-stale"]

    checks = {check.name: check for check in doctor_result.item.checks}
    assert doctor_result.item.status.value == "warn"
