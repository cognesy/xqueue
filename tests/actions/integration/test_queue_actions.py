...
def test_queue_actions_list_and_stats_include_job_counts(tmp_path: Path) -> None:
    ...
    assert [item.name for item in listed.items] == ["alpha", "beta"]
    alpha_stats = next(item for item in stats.items if item.name == "alpha")
    beta_stats = next(item for item in stats.items if item.name == "beta")
...
def test_queue_stats_counts_timed_out_and_dead_jobs(tmp_path: Path) -> None:
    ...
    gamma_stats = next(item for item in stats.items if item.name == "gamma")
...
def test_paused_queue_blocks_claims_until_resumed(tmp_path: Path) -> None:
    ...
    assert pause_result.item.state.value == "paused"

    blocked = worker_action(
        RegisterWorkerInput(worker_id="worker-1", queues=["alpha"]),
        lease_duration_seconds=30,
    )
    assert blocked.item.claimed_job is None

    resume_result = resume_action("alpha")
    assert resume_result.item.state.value == "active"

    claimed = worker_action(
        RegisterWorkerInput(worker_id="worker-1", queues=["alpha"]),
        lease_duration_seconds=30,
    )
    assert claimed.item.claimed_job is not None
    assert claimed.item.claimed_job.id == "job-1"
...
def test_purge_jobs_action_deletes_only_queued_and_retry_scheduled_jobs(tmp_path: Path) -> None:
    ...
    assert result.item.queue == "alpha"
    assert result.item.deleted_count == 2
...
def test_purge_jobs_succeeds_when_retry_scheduled_job_has_attempts(tmp_path: Path) -> None:
    ...
    assert result.item.queue == "alpha"
    assert result.item.deleted_count == 1

    with engine.connect() as connection:
        from sqlalchemy import text
        assert connection.execute(text("SELECT COUNT(*) FROM jobs WHERE queue = 'alpha'")).scalar() == 0
        assert connection.execute(text("SELECT COUNT(*) FROM attempts WHERE job_id = 'job-retry-with-attempt'")).scalar() == 0
        assert connection.execute(text("SELECT COUNT(*) FROM events WHERE job_id = 'job-retry-with-attempt'")).scalar() == 0
