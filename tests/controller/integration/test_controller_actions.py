...
from xqueue.controller.models import ControllerState
...
def test_run_controller_action_restarts_failed_pool_worker(tmp_path: Path) -> None:
    ...
    assert result.state == ControllerState.STOPPED
    assert len(launch_calls) >= 2
    assert launch_calls[0][1:3] == ["-m", "xqueue_cli"]
    assert result.pools[0].workers[0].restart_count >= 1
...
def test_controller_pool_actions_preserve_known_config_and_are_idempotent(tmp_path: Path) -> None:
    ...
    assert noop.restart_required is False
    assert [item.name for item in listed] == ["existing", "xpm"]
    assert removed.action == "removed"
    assert removed.restart_required is True
    assert removed_again.action == "noop"
