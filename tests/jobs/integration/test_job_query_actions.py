...
from xqueue.jobs.actions import ListJobsAction, ShowJobAction, TailJobLogsAction
from xqueue.jobs.logs import JobLogService
from xqueue.jobs.models import AttemptLogStream, JobListFilters, JobListSort
...
def test_list_jobs_action_filters_and_sorts_jobs(tmp_path: Path) -> None:
    ...
    assert [item.id for item in result] == ["job-3", "job-2"]
    assert [item.state.value for item in result] == ["queued", "queued"]
...
def test_show_job_action_returns_detail_with_attempts(tmp_path: Path) -> None:
    ...
    assert result.attempts[0].stderr_path == "/tmp/stderr.log"
...
def test_tail_job_logs_action_returns_latest_attempt_stderr(tmp_path: Path) -> None:
    ...
    assert result.job_id == "job-tail"
    assert result.path == str(stderr_path)
