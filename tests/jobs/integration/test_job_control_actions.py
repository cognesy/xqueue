...
from xqueue.adapters.sqlite.models import AttemptModel, Base, JobModel
from xqueue.adapters.sqlite.session import SessionManager
from xqueue.core.errors import ConflictError
from xqueue.jobs.actions import CancelJobAction, DeleteJobAction, RetryJobAction
...
def test_cancel_job_action_cancels_queued_job(tmp_path: Path) -> None:
    ...
    assert result.cancel_requested_at == now
...
def test_cancel_job_action_requests_running_job_cancellation_and_worker_enforces_it(tmp_path: Path) -> None:
    ...
    assert cancel_result.id == "job-running"
    assert cancel_result.cancel_requested_at == now

    worker_thread.join(timeout=5)
    assert not worker_thread.is_alive()

    final_result = container["result"]
    assert final_result.claimed_job is not None
    assert final_result.claimed_job.state.value == "canceled"
    assert final_result.claimed_job.attempts[0].state.value == "canceled"
    assert final_result.claimed_job.attempts[0].cancellation_reason == "operator_requested"
...
def test_retry_job_action_requeues_terminal_job_without_erasing_attempts(tmp_path: Path) -> None:
    ...
    assert result.state.value == "queued"
...
def test_delete_job_action_removes_non_running_job_history_and_logs(tmp_path: Path) -> None:
    ...
    assert result.job_id == "job-delete"
    assert result.deleted_state.value == "failed"
    assert result.deleted_attempt_count == 1
    assert sorted(result.deleted_log_paths) == sorted([str(stdout_path), str(stderr_path)])
    assert not stdout_path.exists()
    assert not stderr_path.exists()

    with engine.connect() as connection:
        job_count = connection.execute(
            text("SELECT COUNT(*) AS count FROM jobs WHERE id = :job_id"), {"job_id": "job-delete"}
        ).one()
...
def test_retry_job_action_rejects_non_terminal_states(tmp_path: Path) -> None:
    ...
    for state in ("running", "queued", "retry_scheduled", "succeeded"):
        ...
        from xqueue.core.errors import ConflictError as _ConflictError
