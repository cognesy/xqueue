...
from xqueue.workers.orchestration import RunWorkerAction, RunWorkerLoopAction
...
def _build_action(
    *,
    ...
    return RunWorkerAction(
        ...
        log_root=log_root,
        metrics=MetricsService(log_root.parent / "metrics.json"),
...
def test_run_worker_action_executes_successful_job_and_records_attempt(tmp_path: Path) -> None:
    ...
    assert result.claimed_job.state.value == "succeeded"
    assert len(result.claimed_job.attempts) == 1
    assert Path(result.claimed_job.attempts[0].stdout_path or "").read_text() == "success\n"
    ...
    operation_log_path = Path(result.claimed_job.attempts[0].event_log_path or "")
...
def test_run_worker_action_schedules_retry_then_succeeds_on_second_attempt(tmp_path: Path) -> None:
    ...
    assert first_result.claimed_job is not None
    assert first_result.claimed_job.state.value == "retry_scheduled"
    assert len(first_result.claimed_job.attempts) == 1
    assert first_result.claimed_job.attempts[0].state.value == "failed"
    assert first_result.claimed_job.available_at is not None
    first_operation_log = Path(first_result.claimed_job.attempts[0].event_log_path or "")
    ...
    second_action = _build_action(
        session_factory=session_factory,
        log_root=tmp_path / "logs",
        clock=lambda: first_result.claimed_job.available_at + timedelta(seconds=1),
        retry_delay_seconds=3,
    )
    ...
    assert second_result.claimed_job is not None
    assert second_result.claimed_job.state.value == "succeeded"
    assert len(second_result.claimed_job.attempts) == 2
    assert second_result.claimed_job.attempts[1].state.value == "succeeded"
    ...
    with engine.connect() as connection:
        ...
        attempt_rows = connection.execute(
            text(
                "SELECT attempt_number, state, exit_code FROM attempts WHERE job_id = :job_id ORDER BY attempt_number"
            ),
            {"job_id": "job-retry"},
        ).all()
...
def test_run_worker_action_marks_timeout_and_final_failure(tmp_path: Path) -> None:
    ...
    assert result.claimed_job is not None
    assert result.claimed_job.state.value == "failed"
    assert result.claimed_job.last_error == "timed out"
    assert result.claimed_job.attempts[0].state.value == "timed_out"
...
def test_run_worker_loop_action_executes_multiple_jobs_concurrently(tmp_path: Path) -> None:
    ...
    with engine.connect() as connection:
        job_rows = connection.execute(
            text("SELECT id, state, attempt_count FROM jobs WHERE id LIKE 'job-concurrency-%' ORDER BY id")
        ).all()
        attempt_rows = connection.execute(
            text("SELECT job_id, state FROM attempts WHERE job_id LIKE 'job-concurrency-%' ORDER BY job_id")
        ).all()
